...
import os
try:
    from .mnist_model import mnist_fc
...
y_fc = mnist_fc(X, 10, keep_prob)
y_preds = tf.nn.softmax(y_fc)

ff_loss = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits_v2(logits=y_fc, labels=Y))
